...
$$
\Delta \theta_t=\rho \Delta \theta_{t-1} -\alpha g_{t}(\theta_{t-1}+\rho \Delta \theta_{t-1}) \\
...
$$
\theta*=\arg\min_{\theta} \frac{1}{N}\sum_{n=1}^{N} \mathcal{L}(y^{(n)},f(x^{(n)},\theta))+\lambda\mathcal{l}_{p}(\theta)
$$
